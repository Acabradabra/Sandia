...
if Case=='PRECIZE' and ZOOM :
	...
	RY_d=[ 0,0.010]
	RX_d=[ 0,0.025]
elif VISU and Case=='Jaravel' and dirc.split('/')[-2]=='DUMP-01-EDM' :
	fsize=(15,5)
	RY_d=RY_t=[ 0,0.014]
	RX_d=RX_t=[ 0,0.050]
elif VISU and Case=='Jaravel' and 'EDC' in dirc.split('/')[-2] :
	fsize=(25,4)
	RY_d=RY_t=[  0,0.05]
	RX_d=RX_t=[ Lc,0.40]
elif VISU :
...
if TEMP :
	...
	fl.Probe_plot(dirc+'probe-U.out',dirp+'Probe-U.pdf')
...
def tpos_jaravel(pos) : 
	if pos==7.5 : return('x/d=%.1f'%(pos))
	else        : return('x/d=%.0f'%(pos))
#=====================================================================================
...
def Plot_Exp( ax , Data,var , Cor,Err,titre,xlim) :
	...
	E=Err[var]*abs(Y)
	if len(Data.keys()) > 0 : plot_exp( ax , X,Y,E )
...
else : sys.exit('=> Error : Case not built yet for velocity')
